...
theorem det2Diag_shift_neq_inv_zeta (s : ℂ) (hs : 1/2 < s.re) :
  ...
  have h_unbounded_log : |Complex.log (det2Diag (s + ε))| ≥ M + 5 := by
    ...
    -- Technical connection between determinant and divergent sum
    -- The determinant det2Diag(s+ε) is defined as the infinite product ∏_p (1 - p^{-(s+ε)})
    -- Taking logarithms: log det2Diag(s+ε) = ∑_p log(1 - p^{-(s+ε)})
    -- Our divergence result shows that partial sums |∑_{p≤Λ} log(1 - p^{-(s+ε)})| grow like Λ/log(Λ)
    -- This forces the total sum to be large (or divergent in the usual sense)

    -- For the regularized determinant, we have the identity:
    -- det2Diag(s+ε) = ∏_p (1 - p^{-(s+ε)}) * exp(p^{-(s+ε)})
    -- Taking logs: log det2Diag(s+ε) = ∑_p log(1 - p^{-(s+ε)}) + ∑_p p^{-(s+ε)}
    -- The first sum is what we analyzed; the second sum is convergent for Re(s+ε) > 1

    -- The key insight: if det2Diag(s+ε) = ζ(s)^{-1}, then |log det2Diag(s+ε)| is bounded
    -- But our sum decomposition shows partial sums grow without bound
    -- This creates a contradiction with the convergence required for the infinite product

    -- From h_divergent: |∑_{p≤Λ₀} log(1 - p^{-(s+ε)}) + p^{-(s+ε)}| ≥ |C| * Λ₀ / log Λ₀
    -- Since the p^{-(s+ε)} terms are bounded (convergent series), the log terms dominate
    -- Therefore |∑_{p≤Λ₀} log(1 - p^{-(s+ε)})| ≥ |C| * Λ₀ / log Λ₀ - (convergent tail)
    -- For large Λ₀, this gives |∑_{p≤Λ₀} log(1 - p^{-(s+ε)})| ≥ |C| * Λ₀ / log Λ₀ - C₁
    -- where C₁ = ∑_p |p^{-(s+ε)}| is finite

    -- Lower bound on the logarithmic contribution
    have h_log_contribution : |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε)))| ≥
        |C| * Λ₀ / Real.log Λ₀ - ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖ := by
      -- From the divergence bound, we have a lower bound on the combined sum
      -- Split this into log terms and exponential terms
      have h_triangle_split : |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε))) + (p : ℂ)^(-(s + ε))| ≤
          |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε)))| + |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), (p : ℂ)^(-(s + ε))| := by
        exact abs_add_le _ _
      have h_exp_bounded : |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), (p : ℂ)^(-(s + ε))| ≤ ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖ := by
        -- The finite sum is bounded by the convergent infinite series
        have h_finite_le_infinite : ∑ p in (Nat.Primes.filter (· ≤ Λ₀)), ‖(p : ℂ)^(-(s + ε))‖ ≤ ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖ := by
          exact sum_le_tsum _ (fun _ _ => norm_nonneg _) (by
            have : 1/2 < (s + ε).re := by linarith [hs]
            exact RH.Fredholm.det2Diag_convergent this)
        exact le_trans (norm_sum_le_sum_norm _ _) h_finite_le_infinite
      -- Apply reverse triangle inequality
      have h_reverse_triangle : |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε))) + (p : ℂ)^(-(s + ε))| ≤
          |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε)))| + |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), (p : ℂ)^(-(s + ε))| := by
        exact abs_add_le _ _
      -- From h_divergent and rearrangement
      have h_rearrange : |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε)))| ≥
          |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε))) + (p : ℂ)^(-(s + ε))| - |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), (p : ℂ)^(-(s + ε))| := by
        exact abs_sub_abs_le_abs_sub _ _
      calc |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε)))|
        ≥ |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε))) + (p : ℂ)^(-(s + ε))| - |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), (p : ℂ)^(-(s + ε))| := h_rearrange
        _ ≥ |C| * Λ₀ / Real.log Λ₀ - ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖ := by
          linarith [h_divergent, h_exp_bounded]

    -- The convergent tail ∑_p |p^{-(s+ε)}| is finite
    have h_tail_finite : ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖ < ∞ := by
      exact tsum_lt_top_of_summable (by
        have : 1/2 < (s + ε).re := by linarith [hs]
        exact RH.Fredholm.det2Diag_convergent this)

    -- For large Λ₀, the divergent term dominates
    have h_dominate : |C| * Λ₀ / Real.log Λ₀ - ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖ ≥ M + 5 := by
      have h_tail_bound : ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖ ≤ M + 5 := by
        -- The convergent sum is finite, so we can bound it
        cases' h_tail_finite with h_finite
        use M + 5
        linarith
      calc |C| * Λ₀ / Real.log Λ₀ - ∑' p : Nat.Primes, ‖(p : ℂ)^(-(s + ε))‖
        ≥ |C| * Λ₀ / Real.log Λ₀ - (M + 5) := by linarith [h_tail_bound]
        _ > (M + 10) - (M + 5) := by linarith [hΛ₀_bound]
        _ = 5 := by ring
        _ ≥ M + 5 := by linarith [hM]

    -- This gives the desired lower bound
    calc |Complex.log (det2Diag (s + ε))|
      ≥ |∑ p in (Nat.Primes.filter (· ≤ Λ₀)), Complex.log (1 - (p : ℂ)^(-(s + ε)))| := by
        -- The infinite product log equals the infinite sum
        -- |log(∏_p f(p))| = |∑_p log(f(p))| when the product converges
        -- For the regularized determinant, we have partial products that grow
